...
class NewsletterOrderForm extends FormBase {
    ...
    public function buildForm(array $form, FormStateInterface $form_state) {
        ...
        $form['newsletter_order']['category'] = [
            ...
            '#description' => $this->t('Bitte wählen Sie die Kategorien, über die Sie künftig per Newsletter informiert werden möchten.'),
            '#options' => $this->getContentTypes(),
            '#required' => TRUE,
        ];

        $form['newsletter_order']['interval'] = [
            '#type' => 'select',
            '#title' => $this->t('Intervall'),
            '#options' => [
              1 => $this->t('wöchentlich'),
              4 => $this->t('monatlich'),
              12 => $this->t('vierteljährlich'),
            ],
            '#description' => $this->t('In welchem Intervall wünschen Sie den Newsletter?'),
    ...
    public function submitForm(array &$form, FormStateInterface $form_state) {
        ...
        $branch = ($branch === '- Auswählen -') ? '' : $branch;
        ...
        $this->saveToDatabase($salutation, $firstname, $surname, $email, $company, $branch, $types, $token, $interval);
    ...
    public function saveToDatabase($salutation, $firstname, $surname, $email, $company, $branch, $types, $token, $interval) {
        $connection = \Drupal::service('database');
        $newsHelper = new NewsHelper();
        $lastACtivity = $newsHelper->getLatestActivityIdFromLog();
        try {
    ...
    public function getContentTypes() {
        ...
        $types = array_intersect($categories, $categories2);
        asort($types);
        return $types;
        //return $config_types;
    }
